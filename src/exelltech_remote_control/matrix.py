class Matrix:
    ...
    def __init__(self, input_channels: int, output_channels: int) -> None:
        ...
        self._matrix_gains = [[0.0 for _ in range(output_channels)] for _ in range(input_channels)]
    ...
    def set_route(self, row: int, col: int, value: bool) -> None:
        ...
        if not (0 <= row < self._input_channels) or not (0 <= col < self._output_channels):
            raise ValueError("Row and column indices must be between 0 and input/output channels.")
    ...
    def __repr__(self) -> str:
        description = (
            f"Matrix with {self._input_channels} input channels and {self._output_channels} output channels:\n"
        )
